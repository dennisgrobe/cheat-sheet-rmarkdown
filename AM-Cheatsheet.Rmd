...
#### 2. Plotting Portfolio Return against Market Return
...
There is evidence of timing ability **if the coefficient $\boldsymbol{\gamma_p}$ is positive** (and statistically significant). Unfortunately, the power of these tests is low.
...
s_t &= ln(S_t) \qquad |\text{ Note: $S_t$ in Price Quotation} \\
\end{aligned}
\end{equation}
...
>>>>>>> 42cb73bed947c396516ee03c17715204bad5e2f0
...
\begin{equation}
\begin{aligned}
\alpha_p &= \mathbf{w' \boldsymbol{\alpha_i}} \\
\beta_p &= \mathbf{w' \boldsymbol{\beta_i}} \\
\rho_{X,Y} &= \frac{\beta_X\beta_Y\sigma_M^2}{\sigma_X \cdot\sigma_Y} = \frac{Cov(X,Y)}{\sigma_X \cdot\sigma_Y} \\
Cov(X,Y) &= \beta_X\beta_Y\sigma_M^2 = \frac{Cov(X,M) Cov(X,M)}{\sigma_M^2} \\
...
Or as sum of idiosyncratic variances:
\begin{boldmath}
\begin{equation}
\begin{aligned}
\sigma_p^2 &= \beta_p^2 \sigma_M^2 + \sum w_i^2 \sigma_{\epsilon, i}^2
\end{aligned}
\end{equation}
\end{boldmath}
where $\sigma_{\epsilon, i}^2 = \sigma_{i}^2 - \beta_i^2 \sigma_M^2$

\

If two assets are uncorrelated, the combined Sharpe Ratio is:

\begin{equation}
\begin{aligned}
SR_p  &= \sqrt{SR_1^2 + SR_2^2}
\end{aligned}
\end{equation}
...
\begin{equation}\begin{aligned}
R^2 = \frac{SS_{regressor}}{SS_{total}}
\end{aligned}\end{equation}
=======
